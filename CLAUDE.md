...
- Converting the records of an earlier run or an archive to the current shape (version 2):
  `node /opt/plan-review/src/convert.ts <plan-review directory>`; idempotent, never touches the agents' raw replies.
...
| File | Content |
|---|---|
| `src/main.ts` | Entry point: the live wiring and the platform runner (`NodeRuntime.runMain`) applied to the program; untested, like `src/sdkLive.ts` and `src/convert.ts` |
| `src/convert.ts` | Entry point of the archive converter (`convertPlanReviewDir` of `src/records.ts` applied to one directory); untested wiring |
...
- `src/claude.ts` and `src/codex.ts` receive the SDKs through the `Sdk` service and are tested with `test/fakeSdk.ts`. Only `src/sdkLive.ts` (the binding), `src/main.ts` (the wiring and the runner) and `src/convert.ts` (the converter's entry point) are untested; their text is shown to the developer before it is written. No scaffolding may reach a real agent: a scaffold of an adapter makes the SDK call impossible. Verify SDK option names against the type declarations in `node_modules`, not from memory.
