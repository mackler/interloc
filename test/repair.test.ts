...
// The same behaviour through the real adapters and the fake SDKs. The adapters still take a State
// (until plan step 5.4); the procedure uses the Store on the platform services.
const store = (state: State) => Layer.provide(storeLayer(state.project, []), platformLayer);
...
const withCodex = (state: State, sdk: FakeSdk, planner: ScriptedPlanner): Layer.Layer<Services> =>
  Layer.mergeAll(store(state), uiLayer(new ScriptedUi([])), plannerLayer(planner), reviewerLayer(new CodexReviewer(state, config, sdk)), Layer.succeed(RunConfig, config));
...
test("a Codex reply without an issues array gets one repair turn in the same thread and the corrected review is used", async () => {
  ...
  assert.equal(kept(state.dir, "codex-1.json"), '{"x":1}');
});
...
const withClaude = (state: State, sdk: FakeSdk, reviewer: ScriptedReviewer): Layer.Layer<Services> => {
  ...
  return Layer.mergeAll(store(state), uiLayer(ui), plannerLayer(new ClaudePlanner(state, ui, config, sdk)), reviewerLayer(reviewer), Layer.succeed(RunConfig, config));
};
...
test("a Claude Code planning call without structured output gets one repair turn in the same session", async () => {
  ...
  assert.equal(kept(state.dir, "claude-1.json"), "null");
});
